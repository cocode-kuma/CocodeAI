/**
 * CocodeAI OpenAI OAuth REST API
 *
 * POST   /api/cocodeai-openai-oauth/start    — 生成 PKCE+state,返回 authorize URL
 * GET    /auth/callback                  — 用户浏览器 redirect 到此,完成 token 交换
 * GET    /callback/openai                — 兼容旧路径
 * GET    /api/cocodeai-openai-oauth          — 查询当前登录状态(不回传 token 本体)
 * DELETE /api/cocodeai-openai-oauth          — 登出,删除 token 文件
 */
...
export async function handleCocodeAIOpenAIOAuthApi(
  req: Request,
  url: URL,
  segments: string[],
): Promise<Response> {
  try {
    const action = segments[2] // segments: ['api', 'cocodeai-openai-oauth', <action?>]
    ...
    if (action === 'start' && req.method === 'POST') {
      ...
      const session = await cocodeAIOpenAIOAuthService.startSession({
    ...
    if (action === undefined && req.method === 'GET') {
      const tokens = await cocodeAIOpenAIOAuthService.ensureFreshTokens()
      if (!tokens) {
    ...
    if (action === undefined && req.method === 'DELETE') {
      await cocodeAIOpenAIOAuthService.deleteTokens()
...
export async function handleCocodeAIOpenAIOAuthCallback(url: URL): Promise<Response> {
  ...
  try {
    await cocodeAIOpenAIOAuthService.completeSession(code, state)
...
</head><body><div class="card"><h1>✓ OpenAI Login Successful</h1><p>You can close this window and return to CocodeAI.</p></div>
