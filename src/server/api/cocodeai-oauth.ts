/**
 * CocodeAI OAuth REST API
 *
 * POST   /api/cocodeai-oauth/start    — 生成 PKCE+state,返回 authorize URL
 * GET    /callback                — 用户浏览器 redirect 到此,完成 token 交换
 * GET    /api/cocodeai-oauth/callback — 兼容旧路径
 * GET    /api/cocodeai-oauth          — 查询当前登录状态(不回传 token 本体)
 * GET    /api/cocodeai-oauth/status   — 同上(legacy path)
 * DELETE /api/cocodeai-oauth          — 登出,删除 token 文件
 */

import { z } from 'zod'
import { cocodeAIOAuthService } from '../services/CocodeAIOAuthService.js'
...
export async function handleCocodeAIOAuthApi(
  req: Request,
  url: URL,
  segments: string[],
): Promise<Response> {
  try {
    const action = segments[2] // segments: ['api', 'cocodeai-oauth', <action?>]
    ...
    if (action === 'start' && req.method === 'POST') {
      ...
      const session = cocodeAIOAuthService.startSession({
    ...
    if (action === 'callback' && req.method === 'GET') {
      return handleCocodeAIOAuthCallback(url)
    }

    if ((action === undefined || action === 'status') && req.method === 'GET') {
      const tokens = await cocodeAIOAuthService.ensureFreshTokens()
      if (!tokens) {
    ...
    if (action === undefined && req.method === 'DELETE') {
      await cocodeAIOAuthService.deleteTokens()
...
export async function handleCocodeAIOAuthCallback(url: URL): Promise<Response> {
  ...
  try {
    await cocodeAIOAuthService.completeSession(code, state)
...
</head><body><div class="card"><h1>✓ Login Successful</h1><p>You can close this window and return to CocodeAI.</p></div>
