  ...
  private async buildTargets(): Promise<DoctorTarget[]> {
    const targets: DoctorTarget[] = [
      ...
      this.jsonTarget(
        ...
        'Managed providers',
        'user',
        path.join(this.configDir, 'cocodeai', 'providers.json'),
      ),
      this.jsonTarget(
        'cocodeai-settings',
        'Managed provider settings',
        'user',
        path.join(this.configDir, 'cocodeai', 'settings.json'),
      ),
      ...
      this.jsonTarget('oauth', 'OAuth tokens', 'user', path.join(this.configDir, 'cocodeai', 'oauth.json')),
      this.jsonTarget(
        'openai-oauth',
        'OpenAI OAuth tokens',
        'user',
        path.join(this.configDir, 'cocodeai', 'openai-oauth.json'),
      ),
    ]
