...
export function resolveCronProjectRoot(
  ...
  const explicitRoot = env.COCODEAI_ROOT?.trim()
...
export class CronScheduler {
  ...
  private shouldStripInheritedProviderEnv(providerId?: string | null): boolean {
    ...
    const CocodeAIDir = path.join(this.getConfigDir(), 'cocodeai')
    if (existsSync(path.join(CocodeAIDir, 'providers.json'))) {
      return true
    }

    try {
      const raw = readFileSync(path.join(CocodeAIDir, 'settings.json'), 'utf-8')
  ...
  private shouldMarkManagedOAuth(providerId?: string | null): boolean {
    ...
    try {
      const raw = readFileSync(
        path.join(this.getConfigDir(), 'cocodeai', 'settings.json'),
        'utf-8',
      )
      const parsed = JSON.parse(raw) as { env?: Record<string, string> }
      const env = parsed.env ?? {}
