...
export class ConversationService {
  ...
  private async buildChildEnv(
    ...
    // from ~/.claude/cocodeai/settings.json instead of stale process.env.
    //
    ...
    const PROVIDER_ENV_KEYS = [
      ...
      'COCODEAI_SEND_DISABLED_THINKING',
    ...
    return {
      ...
      PWD: workDir,
      ...(sdkUrl
        ? { COCODEAI_COMPUTER_USE_HOST_BUNDLE_ID: 'com.cocodeai.desktop' }
        : {}),
      ...(desktopServerUrl
        ? { COCODEAI_DESKTOP_SERVER_URL: desktopServerUrl }
        : {}),
      ...(sdkUrl
        ? {
            COCODEAI_DESKTOP_AWAIT_MCP: '1',
            COCODEAI_DESKTOP_AWAIT_MCP_TIMEOUT_MS: '5000',
          }
        : {}),
      ...
      COCODEAI_SKIP_DOTENV: '1',
      ...(explicitProviderEnv
        ? { CLAUDE_CODE_PROVIDER_MANAGED_BY_HOST: '1' }
        : {}),
      // "官方" 模式 (cocodeai/settings.json 没 provider env) 下,把 CLI 标记为
   ...
   * - 如果 cocodeai 自管的 oauth.json 里有可用 token,注入 CLAUDE_CODE_OAUTH_TOKEN
  ...
  private async buildOfficialOAuthEnv(): Promise<Record<string, string>> {
    ...
    try {
      ...
      const { cocodeAIOAuthService } = await import('./CocodeAIOAuthService.js')
      const token = await cocodeAIOAuthService.ensureFreshAccessToken()
      if (token) {
  ...
  private shouldStripInheritedProviderEnv(providerId?: string | null): boolean {
    ...
    const CocodeAIDir = path.join(configDir, 'cocodeai')
    const providersIndexPath = path.join(CocodeAIDir, 'providers.json')
    const settingsPath = path.join(CocodeAIDir, 'settings.json')
   ...
   * 强制切 OAuth。
   *
   * 默认 (读不到 settings.json) 按"官方"处理 — 即使用户从未用过 cocodeai
  ...
  private shouldMarkManagedOAuth(providerId?: string | null): boolean {
    ...
    const configDir =
      process.env.CLAUDE_CONFIG_DIR || path.join(os.homedir(), '.claude')
    const settingsPath = path.join(configDir, 'cocodeai', 'settings.json')
    try {
      const raw = fs.readFileSync(settingsPath, 'utf-8')
      const parsed = JSON.parse(raw) as { env?: Record<string, string> }
      const env = parsed.env ?? {}
  ...
  private resolveCliArgs(baseArgs: string[]): string[] {
    ...
    if (!launcher) {
      ...
      return [path.resolve(import.meta.dir, '../../../bin/cocodeai'), ...baseArgs]
    }
  ...
  private buildStartupError(
    ...
    if (
      ...
      return new ConversationStartupError(
        'Desktop chat could not start because Claude CLI is not authenticated. Run `./bin/cocodeai /login` or provide valid API credentials, then retry.',
        'CLI_AUTH_REQUIRED',
      )
    }
