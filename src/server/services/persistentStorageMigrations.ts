...
async function migrateLegacyRootProviders(
  ...
  const targetPath = path.join(CocodeAIDir, 'providers.json')
  try {
    await fs.access(targetPath)
    return
  } catch (error) {
    if (errnoCode(error) !== 'ENOENT') {
      report.failures.push(`cocodeai/providers.json: ${error instanceof Error ? error.message : String(error)}`)
      return
    }
  }
  ...
  try {
    ...
    report.migratedEntries.push('providers.json -> cocodeai/providers.json')
    ...
    if (managedSettings) {
      ...
      report.migratedEntries.push('providers.json -> cocodeai/settings.json')
    }
  } catch (error) {
    if (error instanceof SyntaxError) {
