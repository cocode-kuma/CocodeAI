 ...
 *
 * Storage: ~/.claude/cocodeai/providers.json (lightweight index)
 * Active provider env vars written to ~/.claude/cocodeai/settings.json
...
import { cocodeAIOpenAIOAuthService } from './CocodeAIOpenAIOAuthService.js'
import {
...
export class ProviderService {
  ...
  private getCocodeAIDir(): string {
    return path.join(this.getConfigDir(), 'cocodeai')
  }

  private getIndexPath(): string {
    return path.join(this.getCocodeAIDir(), 'providers.json')
  }
   ...
   *  1. A cocodeai provider is active → has auth
  ...
  async checkAuthStatus(): Promise<{
    hasAuth: boolean
    source: 'cocodeai-provider' | 'openai-oauth' | 'original-settings' | 'env' | 'none'
    activeProvider?: string
  }> {
    // 1. Check cocodeai active provider
    const index = await this.readIndex()
    if (index.activeId) {
      ...
      if (provider) {
        ...
        if (Object.values(authEnv).some(value => value.length > 0)) {
          return { hasAuth: true, source: 'cocodeai-provider', activeProvider: provider.name }
        }
      }
    }
