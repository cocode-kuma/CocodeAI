...
async function defaultConvertIconToPng(iconPath: string, size: number): Promise<Uint8Array> {
  ...
  const tmpDir = await mkdtemp(join(tmpdir(), 'cocodeai-open-target-icon-'))
...
$source = $env:COCODEAI_ICON_SOURCE
$output = $env:COCODEAI_ICON_OUTPUT
...
`
  ...
  await execFile('powershell.exe', [
    ...
    env: {
      ...process.env,
      COCODEAI_ICON_SOURCE: iconPath,
      COCODEAI_ICON_OUTPUT: outputPath,
    },
    timeout: 5_000,
    windowsHide: true,
  })
