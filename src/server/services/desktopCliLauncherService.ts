...
const DESKTOP_CLI_NAME = 'cocodeai'
const DESKTOP_CLI_WINDOWS_LEGACY_EXE = `${DESKTOP_CLI_NAME}.exe`
const PATH_BLOCK_START = '# >>> CocodeAI PATH >>>'
const PATH_BLOCK_END = '# <<< CocodeAI PATH <<<'
...
if [[ ! -x "$SIDECAR" ]]; then
  echo "cocodeai launcher could not find bundled sidecar: $SIDECAR" >&2
  exit 127
fi
...
export function buildWindowsLauncherWrapper(sourcePath: string) {
  ...
  return [
    ...
    '  echo cocodeai launcher could not find bundled sidecar: %SIDECAR% 1>&2',
    '  exit /b 127',
    ')',
