...
export class DiagnosticsService {
  ...
  getLogDir(): string {
    return path.join(this.getConfigDir(), 'cocodeai', 'diagnostics')
  }
  ...
  async exportBundle(): Promise<DiagnosticsExportResult> {
    ...
    const fileName = `cocodeai-diagnostics-${timestamp}.tar.gz`
  ...
  private async buildProvidersSummary(): Promise<Record<string, unknown>> {
    const providerPath = path.join(this.getConfigDir(), 'cocodeai', 'providers.json')
    try {
