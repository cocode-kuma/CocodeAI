/**
 * CocodeAIOAuthService — 桌面端自管 Claude OAuth token
 *
...
export class CocodeAIOAuthService {
  ...
  private getOAuthFilePath(): string {
    ...
    return path.join(configDir, 'cocodeai', 'oauth.json')
  }
  ...
  async ensureFreshTokens(): Promise<StoredOAuthTokens | null> {
    ...
    try {
      ...
      console.error(
        '[CocodeAIOAuthService] token refresh failed:',
...
export const cocodeAIOAuthService = new CocodeAIOAuthService()
