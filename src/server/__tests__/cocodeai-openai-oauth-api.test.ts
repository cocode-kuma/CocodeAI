/**
 * Integration tests for /api/cocodeai-openai-oauth/* endpoints.
 */
...
import { handleCocodeAIOpenAIOAuthApi } from '../api/cocodeai-openai-oauth.js'
import { cocodeAIOpenAIOAuthService } from '../services/CocodeAIOpenAIOAuthService.js'
...
async function setup() {
  tmpDir = await fs.mkdtemp(
    path.join(os.tmpdir(), 'cocodeai-openai-oauth-api-test-'),
  )
...
async function teardown() {
  cocodeAIOpenAIOAuthService.dispose()
  cocodeAIOpenAIOAuthService.resetCallbackPortForTests()
...
describe('POST /api/cocodeai-openai-oauth/start', () => {
  beforeEach(setup)
  afterEach(teardown)

  test('returns authorize URL with PKCE challenge', async () => {
    const callbackPort = await getFreePort()
    cocodeAIOpenAIOAuthService.setCallbackPortForTests(callbackPort)
    ...
    const res = await handleCocodeAIOpenAIOAuthApi(req, url, segments)
...
describe('GET /api/cocodeai-openai-oauth', () => {
  beforeEach(setup)
  afterEach(teardown)

  test('returns loggedIn=false when no token file', async () => {
    const { req, url, segments } = buildReq('GET', '/api/cocodeai-openai-oauth')
    const res = await handleCocodeAIOpenAIOAuthApi(req, url, segments)
    expect(res.status).toBe(200)
  ...
  test('returns loggedIn=true + metadata when token saved', async () => {
    await cocodeAIOpenAIOAuthService.saveTokens({
  ...
  test('returns loggedIn=false when stored token is expired and refresh fails', async () => {
    ...
    cocodeAIOpenAIOAuthService.setRefreshFn(async () => {
...
describe('DELETE /api/cocodeai-openai-oauth', () => {
  beforeEach(setup)
  afterEach(teardown)
  ...
  test('clears token file', async () => {
    ...
    const { req, url, segments } = buildReq('DELETE', '/api/cocodeai-openai-oauth')
    const res = await handleCocodeAIOpenAIOAuthApi(req, url, segments)
    expect(res.status).toBe(200)
    expect(await cocodeAIOpenAIOAuthService.loadTokens()).toBeNull()
  })
})
