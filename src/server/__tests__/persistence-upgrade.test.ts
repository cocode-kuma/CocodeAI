...
describe('persistent storage upgrade migrations', () => {
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'cocodeai-persistence-'))
  ...
  test('migrates legacy providers index and writes a backup before changing it', async () => {
    const CocodeAIDir = path.join(tempDir, 'cocodeai')
    await fs.mkdir(CocodeAIDir, { recursive: true })
    await fs.writeFile(
      path.join(CocodeAIDir, 'providers.json'),
      JSON.stringify({
    ...
    expect(report.migratedEntries).toContain('cocodeai/providers.json')

    const migrated = JSON.parse(await fs.readFile(path.join(CocodeAIDir, 'providers.json'), 'utf-8')) as {
    ...
    const backups = (await listFiles(CocodeAIDir)).filter((file) => file.startsWith('providers.json.bak-before-migration-'))
    ...
    const rewritten = JSON.parse(await fs.readFile(path.join(CocodeAIDir, 'providers.json'), 'utf-8')) as {
      rootFutureField?: unknown
      providers?: Array<Record<string, unknown>>
    }
  ...
  test('imports legacy root providers config into cocodeai storage without deleting the source', async () => {
    await fs.writeFile(
    ...
    expect(report.failures).toEqual([])
    expect(report.migratedEntries).toContain('providers.json -> cocodeai/providers.json')
    expect(report.migratedEntries).toContain('providers.json -> cocodeai/settings.json')
    expect(JSON.parse(await fs.readFile(path.join(tempDir, 'providers.json'), 'utf-8'))).toMatchObject({
      version: 1,
      activeModel: 'legacy-sonnet',
    })

    const migrated = JSON.parse(await fs.readFile(path.join(tempDir, 'cocodeai', 'providers.json'), 'utf-8')) as {
      activeId?: string | null
      providers?: Array<{
        id?: string
        presetId?: string
    ...
    const managedSettings = JSON.parse(await fs.readFile(path.join(tempDir, 'cocodeai', 'settings.json'), 'utf-8')) as {
  ...
  test('does not overwrite current cocodeai provider storage with a legacy root config', async () => {
    const CocodeAIDir = path.join(tempDir, 'cocodeai')
    await fs.mkdir(CocodeAIDir, { recursive: true })
    await fs.writeFile(
      path.join(tempDir, 'providers.json'),
    ...
    expect(report.failures).toEqual([])
    expect(report.migratedEntries).not.toContain('providers.json -> cocodeai/providers.json')
    const current = JSON.parse(await fs.readFile(path.join(CocodeAIDir, 'providers.json'), 'utf-8')) as {
      activeId?: string | null
      providers?: unknown[]
    }
  ...
  test('quarantines malformed managed settings instead of blocking startup', async () => {
    const CocodeAIDir = path.join(tempDir, 'cocodeai')
    await fs.mkdir(CocodeAIDir, { recursive: true })
    await fs.writeFile(path.join(CocodeAIDir, 'settings.json'), '{"env":', 'utf-8')

    const report = await ensurePersistentStorageUpgraded()

    expect(report.failures).toEqual([])
    expect(report.migratedEntries).toContain('cocodeai/settings.json')
    expect(JSON.parse(await fs.readFile(path.join(CocodeAIDir, 'settings.json'), 'utf-8'))).toEqual({})
    const quarantined = (await listFiles(CocodeAIDir)).filter((file) => file.startsWith('settings.json.invalid-'))
  ...
  test('upgrades existing DeepSeek managed env to follow global thinking settings', async () => {
    ...
    await fs.mkdir(CocodeAIDir, { recursive: true })
    await fs.writeFile(
      path.join(CocodeAIDir, 'settings.json'),
      JSON.stringify({
        env: {
          ...
          COCODEAI_SEND_DISABLED_THINKING: '1',
    ...
    const migrated = JSON.parse(await fs.readFile(path.join(CocodeAIDir, 'settings.json'), 'utf-8')) as {
      env?: Record<string, string>
    }
    expect(migrated.env?.COCODEAI_SEND_DISABLED_THINKING).toBeUndefined()
    ...
    const backups = (await listFiles(CocodeAIDir)).filter((file) => file.startsWith('settings.json.bak-before-migration-'))
    expect(backups.length).toBe(1)
  })
})
