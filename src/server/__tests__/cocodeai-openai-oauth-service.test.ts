/**
 * Unit tests for CocodeAIOpenAIOAuthService — cocodeai 自管 OpenAI OAuth 的核心 service 层。
 */
...
import {
  CocodeAIOpenAIOAuthService,
  getCocodeAIOpenAIOAuthFilePath,
  type StoredOpenAIOAuthTokens,
} from '../services/CocodeAIOpenAIOAuthService.js'

let tmpDir: string
let originalConfigDir: string | undefined
let service: CocodeAIOpenAIOAuthService
...
async function setup() {
  tmpDir = await fs.mkdtemp(
    path.join(os.tmpdir(), 'cocodeai-openai-oauth-test-'),
  )
  originalConfigDir = process.env.CLAUDE_CONFIG_DIR
  process.env.CLAUDE_CONFIG_DIR = tmpDir
  callbackPort = await getFreePort()
  service = new CocodeAIOpenAIOAuthService({ callbackPort })
}
...
describe('CocodeAIOpenAIOAuthService — file storage', () => {
  beforeEach(setup)
  afterEach(teardown)
  ...
  test('saveTokens writes file with 0600 permissions', async () => {
    ...
    const oauthPath = getCocodeAIOpenAIOAuthFilePath()
...
describe('CocodeAIOpenAIOAuthService — session management', () => {
  beforeEach(setup)
  afterEach(teardown)
...
describe('CocodeAIOpenAIOAuthService — ensureFreshAccessToken', () => {
  beforeEach(setup)
  afterEach(teardown)
