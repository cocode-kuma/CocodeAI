/**
 * Integration tests for /api/cocodeai-oauth/* endpoints.
 */
...
import { handleCocodeAIOAuthApi } from '../api/cocodeai-oauth.js'
...
async function setup() {
  tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'cocodeai-oauth-api-test-'))
...
describe('POST /api/cocodeai-oauth/start', () => {
  beforeEach(setup)
  afterEach(teardown)

  test('returns authorize URL with PKCE challenge', async () => {
    const { req, url, segments } = buildReq('POST', '/api/cocodeai-oauth/start', {
      serverPort: 54321,
    })
    const res = await handleCocodeAIOAuthApi(req, url, segments)
...
describe('GET /api/cocodeai-oauth/status', () => {
  beforeEach(setup)
  afterEach(teardown)

  test('returns loggedIn=false when no token file', async () => {
    const { req, url, segments } = buildReq('GET', '/api/cocodeai-oauth/status')
    const res = await handleCocodeAIOAuthApi(req, url, segments)
    expect(res.status).toBe(200)
  ...
  test('returns loggedIn=false when stored token is expired and refresh fails', async () => {
    ...
    cocodeAIOAuthService.setRefreshFn(async () => {
...
describe('DELETE /api/cocodeai-oauth', () => {
  beforeEach(setup)
  afterEach(teardown)
  ...
  test('clears token file', async () => {
    ...
    const { req, url, segments } = buildReq('DELETE', '/api/cocodeai-oauth')
    const res = await handleCocodeAIOAuthApi(req, url, segments)
    expect(res.status).toBe(200)
    expect(await cocodeAIOAuthService.loadTokens()).toBeNull()
  })
})
