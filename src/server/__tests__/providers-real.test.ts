 ...
 * 验证添加、激活、cocodeai/settings.json 同步是否正确
 * (provider env 写到 ~/.claude/cocodeai/settings.json，不污染原版 settings.json)
 */
...
describe('Real Provider Configs', () => {
  ...
  // Helper: read the CocodeAI-specific settings file
  async function readCocodeAISettings(): Promise<Record<string, unknown>> {
    const raw = await fs.readFile(path.join(tmpDir, 'cocodeai', 'settings.json'), 'utf-8')
  ...
  test('添加 MiniMax Provider 并激活 — 写入 cocodeai/settings.json', async () => {
    ...
    // 验证写入 cocodeai/settings.json
    const settings = await readCocodeAISettings()
    ...
    console.log('✅ Provider 写入 cocodeai/settings.json，原版 settings.json 未被污染')
  })

  test('切换 Provider — 更新 cocodeai/settings.json', async () => {
    const minimax = await service.addProvider({
      presetId: 'minimax',
      name: 'MiniMax',
      baseUrl: 'https://api.minimaxi.com/anthropic',
    ...
    // 先激活 MiniMax
    await service.activateProvider(minimax.id)
    let settings = await readCocodeAISettings()
    expect((settings.env as Record<string, string>).ANTHROPIC_BASE_URL).toBe('https://api.minimaxi.com/anthropic')
    expect(JSON.parse((settings.env as Record<string, string>).CLAUDE_CODE_MODEL_CONTEXT_WINDOWS)).toMatchObject({
      'MiniMax-M2.7': 204800,
      'MiniMax-M2.7-highspeed': 204800,
    ...
    settings = await readCocodeAISettings()
    ...
    console.log('✅ 切换 Provider 成功，cocodeai/settings.json 更新正确')
  })

  test('cocodeai/settings.json 保留已有字段', async () => {
    // 预写一个有内容的 cocodeai/settings.json（模拟用户已有配置）
    await fs.mkdir(path.join(tmpDir, 'cocodeai'), { recursive: true })
    await fs.writeFile(
      path.join(tmpDir, 'cocodeai', 'settings.json'),
      JSON.stringify({
    ...
    console.log('✅ cocodeai/settings.json 已有字段全部保留')
  })
  ...
  test('providers.json 和 cocodeai/settings.json 独立于 settings.json', async () => {
    ...
    // CocodeAI 添加并激活自己的 provider
    const provider = await service.addProvider({
      presetId: 'minimax',
      name: 'MiniMax',
      baseUrl: 'https://api.minimaxi.com/anthropic',
      apiKey: 'sk-cocodeai-key',
      models: MODEL_MAPPING,
    })
    await service.activateProvider(provider.id)
    ...
    // 验证 cocodeai/settings.json 是 CocodeAI 自己的
    const cocodeai = await readCocodeAISettings()
    expect((cocodeai.env as Record<string, string>).ANTHROPIC_BASE_URL).toBe('https://api.minimaxi.com/anthropic')
    expect((cocodeai.env as Record<string, string>).ANTHROPIC_AUTH_TOKEN).toBe('sk-cocodeai-key')
    expect((cocodeai.env as Record<string, string>).ANTHROPIC_API_KEY).toBe('')

    console.log('✅ 原版 settings.json 完好无损，CocodeAI 配置独立存储')
  })
})
