/**
 * Unit tests for CocodeAIOAuthService — cocodeai 自管 OAuth 的核心 service 层。
 */
...
import {
  CocodeAIOAuthService,
  type StoredOAuthTokens,
} from '../services/CocodeAIOAuthService.js'

let tmpDir: string
let originalConfigDir: string | undefined
let service: CocodeAIOAuthService

async function setup() {
  tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'cocodeai-oauth-test-'))
  originalConfigDir = process.env.CLAUDE_CONFIG_DIR
  process.env.CLAUDE_CONFIG_DIR = tmpDir
  service = new CocodeAIOAuthService()
}
...
describe('CocodeAIOAuthService — file storage', () => {
  beforeEach(setup)
  afterEach(teardown)
  ...
  test('saveTokens writes file with 0600 permissions', async () => {
    ...
    const oauthPath = path.join(tmpDir, 'cocodeai', 'oauth.json')
...
describe('CocodeAIOAuthService — session management', () => {
  beforeEach(setup)
  afterEach(teardown)
...
describe('CocodeAIOAuthService — ensureFreshAccessToken', () => {
  beforeEach(setup)
  afterEach(teardown)
