...
describe('provider presets API', () => {
  ...
  test('configured presets keep current default model ids aligned with official provider docs', () => {
    ...
    expect(deepseek?.defaultEnv?.COCODEAI_SEND_DISABLED_THINKING).toBeUndefined()
    ...
    expect(kimi?.defaultEnv?.COCODEAI_SEND_DISABLED_THINKING).toBe('1')
  ...
  test('configured presets can expose optional API key and promo metadata', () => {
    ...
    expect(zhipu?.promoText).toContain('cocodeai')
    expect(zhipu?.defaultEnv?.COCODEAI_SEND_DISABLED_THINKING).toBe('1')
  ...
  test('GET and PUT /api/providers/settings read and write cocodeai settings.json', async () => {
    const initial = {
      env: {
    ...
    const updatedRaw = await fs.readFile(path.join(tmpDir, 'cocodeai', 'settings.json'), 'utf-8')
