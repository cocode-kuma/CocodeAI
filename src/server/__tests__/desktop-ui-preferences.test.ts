...
async function readDesktopUiFile(): Promise<Record<string, unknown>> {
  const raw = await fs.readFile(path.join(tmpDir, 'cocodeai', 'desktop-ui.json'), 'utf-8')
...
describe('DesktopUiPreferencesService', () => {
  ...
  test('normalizes old schema files and preserves unknown fields when updating sidebar preferences', async () => {
    ...
    await fs.writeFile(
      path.join(tmpDir, 'cocodeai', 'desktop-ui.json'),
      JSON.stringify({
  ...
  test('quarantines corrupt desktop-ui.json and reports defaults as missing', async () => {
    await fs.mkdir(path.join(tmpDir, 'cocodeai'), { recursive: true })
    await fs.writeFile(path.join(tmpDir, 'cocodeai', 'desktop-ui.json'), '{bad json', 'utf-8')
...
describe('desktop UI preferences API', () => {
  ...
  test('persists sidebar preferences under cocodeai desktop-ui.json', async () => {
