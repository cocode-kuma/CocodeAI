...
describe('ConversationService', () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'cocodeai-conversation-service-'))
    ...
    originalDisableTerminalShellEnv = process.env.COCODEAI_DISABLE_TERMINAL_SHELL_ENV
    ...
    process.env.COCODEAI_DISABLE_TERMINAL_SHELL_ENV = '1'
  ...
  afterEach(async () => {
    ...
    if (originalDisableTerminalShellEnv === undefined) delete process.env.COCODEAI_DISABLE_TERMINAL_SHELL_ENV
    else process.env.COCODEAI_DISABLE_TERMINAL_SHELL_ENV = originalDisableTerminalShellEnv
  ...
  test('keeps inherited provider env when no desktop provider config exists', async () => {
    ...
    const env = (await service.buildChildEnv('D:\\workspace\\code\\myself_code\\cocodeai')) as Record<string, string>
    ...
    expect(env.CLAUDE_CODE_DIAGNOSTICS_FILE).toBe(path.join(tmpDir, 'cocodeai', 'diagnostics', 'cli-diagnostics.jsonl'))
    expect(env.CLAUDE_COWORK_MEMORY_PATH_OVERRIDE).toBe(
      `${path.join(tmpDir, 'projects', 'D--workspace-code-myself-code-cocodeai', 'memory')}${path.sep}`,
    )
  ...
  test('buildChildEnv pins desktop memory to the current sanitized project directory', async () => {
    const service = new ConversationService() as any
    const workDir = path.join(tmpDir, 'workspace', 'myself_code', 'cocodeai')
  ...
  test('buildChildEnv inherits exported terminal shell variables for desktop CLI sessions', async () => {
    ...
    delete process.env.COCODEAI_DISABLE_TERMINAL_SHELL_ENV
  ...
  test('strips inherited provider env when desktop provider config exists', async () => {
    const CocodeAIDir = path.join(tmpDir, 'cocodeai')
  ...
  test('buildChildEnv injects CLAUDE_CODE_OAUTH_TOKEN when official mode + cocodeai oauth token exists', async () => {
    const CocodeAIDir = path.join(tmpDir, 'cocodeai')
    await fs.mkdir(CocodeAIDir, { recursive: true })
    await fs.writeFile(
    ...
    const { cocodeAIOAuthService } = await import('../services/CocodeAIOAuthService.js')
    await cocodeAIOAuthService.saveTokens({
      accessToken: 'cocodeai-fresh-token',
      refreshToken: 'cocodeai-refresh-xxx',
    ...
    expect(env.CLAUDE_CODE_OAUTH_TOKEN).toBe('cocodeai-fresh-token')
  })
  ...
  test('buildChildEnv does NOT inject CLAUDE_CODE_OAUTH_TOKEN when not official mode', async () => {
    ...
    const { cocodeAIOAuthService } = await import('../services/CocodeAIOAuthService.js')
    await cocodeAIOAuthService.saveTokens({
      accessToken: 'cocodeai-token-should-not-be-used',
      refreshToken: null,
      expiresAt: null,
      scopes: [],
  ...
  test('buildChildEnv injects ChatGPT Official runtime env for session-scoped provider selection', async () => {
    ...
    expect(env.COCODEAI_OPENAI_OAUTH_PROVIDER).toBe('1')
    expect(env.OPENAI_CODEX_OAUTH_FILE).toBe(
      path.join(tmpDir, 'cocodeai', 'openai-oauth.json'),
    )
  ...
  test('buildChildEnv injects desktop Computer Use host bundle id for sdk sessions', async () => {
    ...
    expect(env.COCODEAI_COMPUTER_USE_HOST_BUNDLE_ID).toBe(
      'com.cocodeai.desktop',
    )
    expect(env.COCODEAI_DESKTOP_SERVER_URL).toBe('http://127.0.0.1:3456')
  ...
  test('uses bun entrypoint fallback on Windows dev mode', () => {
    ...
    if (process.platform === 'win32') {
      ...
      expect(args[0]).toContain(path.join('bin', 'cocodeai'))
    }
  })
  ...
  test('buildChildEnv asks desktop SDK sessions to wait briefly for MCP tools', async () => {
    ...
    expect(env.COCODEAI_DESKTOP_AWAIT_MCP).toBe('1')
    expect(env.COCODEAI_DESKTOP_AWAIT_MCP_TIMEOUT_MS).toBe('5000')
  })
