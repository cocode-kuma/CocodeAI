...
beforeEach(async () => {
  tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'cocodeai-diagnostics-test-'))
...
describe('DiagnosticsService', () => {
  test('writes sanitized structured events and runtime error summaries', async () => {
    ...
    const raw = await fs.readFile(path.join(tmpDir, 'cocodeai', 'diagnostics', 'diagnostics.jsonl'), 'utf-8')
    expect(raw).toContain('cli_start_failed')
    expect(raw).toContain('[REDACTED]')
    expect(raw).not.toContain('sk-secret')
    expect(raw).not.toContain(os.homedir())

    const runtime = await fs.readFile(path.join(tmpDir, 'cocodeai', 'diagnostics', 'runtime-errors.log'), 'utf-8')
  ...
  test('exports a single diagnostics tarball without provider secrets', async () => {
    ...
    await fs.writeFile(
      path.join(tmpDir, 'cocodeai', 'diagnostics', 'cli-diagnostics.jsonl'),
...
describe('diagnostics API', () => {
  test('returns status, events, export path, and supports clearing logs', async () => {
    ...
    expect(status.logDir).toContain(path.join('cocodeai', 'diagnostics'))
