...
/**
 * Read env vars from ~/.claude/cocodeai/settings.json (CocodeAI-specific provider
 ...
 */
function getCocodeAISettingsEnv(): Record<string, string> {
  try {
    const CocodeAISettings = join(getClaudeConfigHomeDir(), 'cocodeai', 'settings.json')
    const raw = readFileSync(CocodeAISettings, 'utf-8')
...
export function applySafeConfigEnvironmentVariables(): void {
  ...
  // cocodeai provider isolation: apply env from ~/.claude/cocodeai/settings.json
  // AFTER userSettings so CocodeAI-specific provider config takes priority over
  // the original Claude Code's settings. This prevents CocodeAI from polluting
  // ~/.claude/settings.json while still allowing it to override provider vars.
  Object.assign(process.env, filterSettingsEnv(getCocodeAISettingsEnv()))
...
export function applyConfigEnvironmentVariables(): void {
  ...
  // cocodeai provider isolation: same as in applySafeConfigEnvironmentVariables,
  // apply CocodeAI-specific env last so it overrides the original settings.
  Object.assign(process.env, filterSettingsEnv(getCocodeAISettingsEnv()))
