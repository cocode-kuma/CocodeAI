...
function getCocodeAIDir(): string {
  const configDir =
    process.env.CLAUDE_CONFIG_DIR || path.join(os.homedir(), '.claude')
  return path.join(configDir, 'cocodeai')
}

function getFileBackedStorageMarkerPath(): string {
  return path.join(getCocodeAIDir(), FILE_BACKED_STORAGE_MARKER_FILE)
}

function markFileBackedStorageUsed(): void {
  try {
    fs.mkdirSync(getCocodeAIDir(), { recursive: true })
