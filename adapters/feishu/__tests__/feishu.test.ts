...
describe('Feishu: project picker card', () => {
  const sampleProjects: RecentProject[] = [
    {
      projectPath: '/Users/dev/cocodeai',
      realPath: '/Users/dev/cocodeai',
      projectName: 'cocodeai',
      isGit: true,
      repoName: 'cocodeai',
      branch: 'main',
  ...
  it('info column has title markdown + notation path markdown', () => {
    ...
    expect(info[0].content).toContain('**cocodeai**')
    ...
    expect(info[1].content).toContain('cocodeai')
  })
  ...
  it('row button says 选择 with small size and carries per-project value', () => {
    ...
    expect(btn1.value.realPath).toBe('/Users/dev/cocodeai')
    expect(btn1.value.projectName).toBe('cocodeai')
...
describe('Feishu: card.action.trigger parsing', () => {
  ...
  it('parses pick_project action from event', () => {
    ...
    expect(event.action.value.realPath).toBe('/Users/dev/cocodeai')
    expect(event.action.value.projectName).toBe('cocodeai')
  })
