...
export async function executeDesktopSmoke(
  ...
  try {
    ...
    const browserSetup = [
      `localStorage.setItem('cocodeai-open-tabs', ${JSON.stringify(JSON.stringify({
        openTabs: [{ sessionId: session.sessionId, title: 'Desktop Smoke', type: 'session' }],
        activeTabId: session.sessionId,
      }))})`,
      runtimeSelection
        ? `localStorage.setItem('cocodeai-session-runtime', ${JSON.stringify(JSON.stringify({
          [session.sessionId]: runtimeSelection,
        }))})`
        : `localStorage.removeItem('cocodeai-session-runtime')`,
    ]
