...
describe('doctorRepair', () => {
  ...
  it('clears only the safe desktop UI storage keys', () => {
    ...
    window.localStorage.setItem('cocodeai-chat-history', 'preserve')
    window.localStorage.setItem('cocodeai-provider-config', 'preserve')
    ...
    expect(window.localStorage.getItem('cocodeai-chat-history')).toBe('preserve')
    expect(window.localStorage.getItem('cocodeai-provider-config')).toBe('preserve')
  })
  ...
  it('keeps local repair successful when the server doctor endpoint is unavailable', async () => {
    ...
    expect(result.local.removedKeys).toContain('cocodeai-theme')
