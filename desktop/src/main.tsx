...
declare global {
  interface Window {
    __COCODEAI_BOOTSTRAPPED__?: boolean
    __COCODEAI_SHOW_STARTUP_ERROR__?: (reason: unknown) => void
  }
}
...
export async function bootstrapDesktopApp(
  ...
  try {
    ...
    window.__COCODEAI_BOOTSTRAPPED__ = true
  } catch (error) {
    console.error('[desktop] Failed to bootstrap app', error)
    if (root) {
      if (window.__COCODEAI_SHOW_STARTUP_ERROR__) {
        window.__COCODEAI_SHOW_STARTUP_ERROR__(error)
      } else {
