...
describe('Settings > General tab', () => {
  beforeEach(() => {
    ...
    tauriDialogMock.open.mockResolvedValue('/Users/test/cocodeai-data')
    ...
    useSettingsStore.setState({
      ...
      appMode: {
        mode: 'default',
        portableDir: null,
        defaultPortableDir: '/Applications/CocodeAI/CLAUDE_CONFIG_DIR',
      ...
      setAppMode: vi.fn().mockImplementation(async (mode: AppMode, portableDir?: string | null) => {
        useSettingsStore.setState({
          appMode: {
            mode,
            portableDir: mode === 'portable' ? portableDir ?? '/Applications/CocodeAI/CLAUDE_CONFIG_DIR' : null,
            defaultPortableDir: '/Applications/CocodeAI/CLAUDE_CONFIG_DIR',
            activeConfigDir: mode === 'portable' ? portableDir ?? '/Applications/CocodeAI/CLAUDE_CONFIG_DIR' : null,
  ...
  it('lets desktop users choose a portable data directory and relaunch immediately', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByLabelText('Portable data directory')).toHaveValue('/Users/test/cocodeai-data')
    })
    ...
    await waitFor(() => {
      expect(useSettingsStore.getState().setAppMode).toHaveBeenCalledWith('portable', '/Users/test/cocodeai-data')
  ...
  it('switches back to the system directory without deleting portable data', async () => {
    ...
    useSettingsStore.setState({
      appMode: {
        mode: 'portable',
        portableDir: '/Users/test/cocodeai-data',
        defaultPortableDir: '/Applications/CocodeAI/CLAUDE_CONFIG_DIR',
        activeConfigDir: '/Users/test/cocodeai-data',
  ...
  it('validates portable directory input and lets users reset to the app-side folder', async () => {
    ...
    expect(input).toHaveValue('/Applications/CocodeAI/CLAUDE_CONFIG_DIR')
...
describe('Settings > About tab', () => {
  ...
  it('renders release notes with markdown formatting', async () => {
    ...
    expect(await screen.findByRole('heading', { name: 'CocodeAI v0.1.5' })).toBeInTheDocument()
