...
describe('Settings > Diagnostics tab', () => {
  beforeEach(() => {
    diagnosticsApiMock.getStatus.mockResolvedValue({
      logDir: '/tmp/claude/cocodeai/diagnostics',
      diagnosticsPath: '/tmp/claude/cocodeai/diagnostics/diagnostics.jsonl',
      cliDiagnosticsPath: '/tmp/claude/cocodeai/diagnostics/cli-diagnostics.jsonl',
      runtimeErrorsPath: '/tmp/claude/cocodeai/diagnostics/runtime-errors.log',
      exportDir: '/tmp/claude/cocodeai/diagnostics/exports',
      retentionDays: 7,
    ...
    diagnosticsApiMock.exportBundle.mockResolvedValue({
      bundle: {
        path: '/tmp/claude/cocodeai/diagnostics/exports/cocodeai-diagnostics.tar.gz',
        fileName: 'cocodeai-diagnostics.tar.gz',
        bytes: 1024,
      },
    })
    ...
    doctorRepairMock.runDoctorRepair.mockResolvedValue({
      local: {
        removedKeys: ['cocodeai-open-tabs', 'cocodeai-session-runtime'],
        missingKeys: ['cocodeai-theme', 'cocodeai-locale', 'cocodeai.persistence.schemaVersion'],
        failedKeys: [],
      },
      server: {
        ok: true,
  ...
  it('shows diagnostics status, actions, and recent events', async () => {
    ...
    expect(screen.getByText('/tmp/claude/cocodeai/diagnostics')).toBeInTheDocument()
  ...
  it('exports a diagnostics bundle from the settings page', async () => {
    ...
    expect(await screen.findByText('/tmp/claude/cocodeai/diagnostics/exports/cocodeai-diagnostics.tar.gz')).toBeInTheDocument()
  })
  ...
  it('runs Doctor from Diagnostics without clearing unrelated desktop state', async () => {
    window.localStorage.setItem('cocodeai-open-tabs', '{"activeTabId":"__settings__"}')
    window.localStorage.setItem('cocodeai-theme', 'dark')
    window.localStorage.setItem('cocodeai-chat-history', 'keep')
    ...
    expect(window.localStorage.getItem('cocodeai-chat-history')).toBe('keep')
  })
})
