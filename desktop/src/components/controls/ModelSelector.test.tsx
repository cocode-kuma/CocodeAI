...
import { useCocodeAIOAuthStore } from '../../stores/CocodeAIOAuthStore'
import { useCocodeAIOpenAIOAuthStore } from '../../stores/CocodeAIOpenAIOAuthStore'
...
afterEach(() => {
  ...
  useCocodeAIOAuthStore.setState(useCocodeAIOAuthStore.getInitialState(), true)
  useCocodeAIOpenAIOAuthStore.setState(useCocodeAIOpenAIOAuthStore.getInitialState(), true)
})

// Prevent real API calls from fetchStatus on mount
beforeEach(() => {
  useCocodeAIOAuthStore.setState({ fetchStatus: async () => {} })
  useCocodeAIOpenAIOAuthStore.setState({ fetchStatus: async () => {} })
})
...
describe('ModelSelector', () => {
  ...
  it('hides official provider sections when OAuth is not logged in', async () => {
    useCocodeAIOAuthStore.setState({ status: { loggedIn: false }, fetchStatus: async () => {} })
    useCocodeAIOpenAIOAuthStore.setState({ status: { loggedIn: false }, fetchStatus: async () => {} })
    useSettingsStore.setState({
      locale: 'en',
      availableModels: MODELS,
      currentModel: MODELS[0],
