...
export function ChatInput({ variant = 'default', compact = false }: ChatInputProps) {
  ...
  return (
    <div
      ...
      className={
        isHeroComposer
          ? `bg-transparent ${isMobileComposer ? 'px-4 pb-3' : 'px-8 pb-6'}`
          : compact
      ...
      >
        <div
          ...
          className={isHeroComposer
            ? `glass-panel relative flex flex-col gap-3 overflow-visible rounded-2xl p-4 transition-colors ${isDragActive ? 'composer-drop-target-active' : ''}`
            : compact
              ? `glass-panel relative overflow-visible p-3 transition-colors ${isMobileComposer ? 'rounded-2xl' : 'rounded-2xl'} ${isDragActive ? 'composer-drop-target-active' : ''}`
              : `glass-panel relative overflow-visible transition-colors ${isMobileComposer ? 'rounded-2xl p-3' : 'rounded-2xl p-4'} ${isDragActive ? 'composer-drop-target-active' : ''}`}
          {...dragHandlers}
        >
          {isDragActive && (
            <ComposerDropOverlay
          ...
          {!isMemberSession && slashMenuOpen && filteredCommands.length > 0 && (
            <div
              ref={slashMenuRef}
              className="absolute bottom-full left-0 right-0 z-50 mb-2 overflow-hidden rounded-[var(--radius-xl)] border border-[var(--color-surface-glass-border)] bg-[var(--color-surface-glass)] [backdrop-filter:blur(40px)_saturate(1.8)] shadow-[var(--shadow-liquid-glass)] animate-liquid-reveal"
            >
              ...
              {!isMobileComposer ? (
                <div className="flex items-center gap-1.5 border-t border-[var(--color-border-separator)] px-4 py-2 text-xs text-[var(--color-text-tertiary)]">
              ...
              {!isMemberSession && (
                <>
                  <div ref={plusMenuRef} className="relative">
                    ...
                    {plusMenuOpen && (
                      <div className={`absolute bottom-full left-0 z-50 mb-2 rounded-[var(--radius-xl)] border border-[var(--color-surface-glass-border)] bg-[var(--color-surface-glass)] [backdrop-filter:blur(40px)_saturate(1.8)] shadow-[var(--shadow-liquid-glass)] animate-liquid-reveal py-1 ${isMobileComposer ? 'w-[min(240px,calc(100vw-32px))]' : 'w-[240px]'}`}>
                        <button
