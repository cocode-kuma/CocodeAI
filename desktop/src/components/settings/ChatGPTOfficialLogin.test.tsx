...
const initialOAuthState = useCocodeAIOpenAIOAuthStore.getState()
...
describe('ChatGPTOfficialLogin', () => {
  ...
  afterEach(() => {
    act(() => {
      ...
      useCocodeAIOpenAIOAuthStore.setState(initialOAuthState)
    })
    vi.useRealTimers()
    cleanup()
    vi.restoreAllMocks()
  ...
  it('keeps an actionable authorization link when shell open fails', async () => {
    ...
    expect(useCocodeAIOpenAIOAuthStore.getState().error).toBeNull()
