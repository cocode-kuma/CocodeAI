...
const CHAT_COLUMN_CENTERED_CLASS = 'mx-auto flex w-full max-w-[1080px] min-w-[360px] flex-col'
...
export function ActiveSession() {
  ...
  const useCenteredEmptyComposer = isEmpty && !isMemberSession && !showWorkspacePanel
  ...
  return (
    <div className="active-session-page flex-1 flex relative overflow-hidden text-on-surface">
      <div data-testid="active-session-content-row" className="flex min-h-0 min-w-0 flex-1">
        <div
          data-testid="active-session-chat-column"
          className={showWorkspacePanel
            ? `flex flex-col ${CHAT_COLUMN_WITH_WORKSPACE_CLASS}`
            : isMobileLayout
              ? 'flex min-w-0 flex-1 flex-col'
              : CHAT_COLUMN_CENTERED_CLASS}
        >
          {isMemberSession && (
          ...
          {isEmpty ? (
            useCenteredEmptyComposer ? (
              <div className="flex flex-1 flex-col items-center justify-center px-4 pb-[18vh] pt-8 md:px-8">
                <ChatInput
                  variant="hero"
                  compact={false}
                />
