...
fn get_app_mode(app: AppHandle) -> serde_json::Value {
    ...
    let config_dir_source = if env_config_dir.is_some() {
        if std::env::var_os("COCODEAI_APP_PORTABLE_DIR").is_some() {
            "portable"
        } else {
            "environment"
        }
...
fn setup_system_tray(app: &mut tauri::App) -> tauri::Result<()> {
    let menu = MenuBuilder::new(app)
        .text(TRAY_SHOW_ID, "Show CocodeAI")
        .separator()
        .text(TRAY_QUIT_ID, "Quit CocodeAI")
        .build()?;
...
pub fn run() {
    ...
    let builder = builder
        .menu(|app| {
            let about_item =
                MenuItemBuilder::with_id("nav_about", "关于 CocodeAI").build(app)?;
            let settings_item = MenuItemBuilder::with_id("nav_settings", "设置...")
                .accelerator("CmdOrCtrl+,")
                .build(app)?;

            let app_submenu = SubmenuBuilder::new(app, "CocodeAI")
                .item(&about_item)
                .separator()
                .item(&settings_item)
                .separator()
