...
ARTIFACT_DIR="${ARTIFACT_DIR:-/tmp/cocodeai-e2e-parallel-providers-$$}"
CONFIG_DIR="${ARTIFACT_DIR}/config"
PROJECT_A="${ARTIFACT_DIR}/project-a"
PROJECT_B="${ARTIFACT_DIR}/project-b"
SESSION_NAME_A="${SESSION_NAME_A:-cocodeai-provider-a-$$}"
SESSION_NAME_B="${SESSION_NAME_B:-cocodeai-provider-b-$$}"
...
mkdir -p "${ARTIFACT_DIR}" "${CONFIG_DIR}/cocodeai" "${PROJECT_A}" "${PROJECT_B}"
...
mkdirSync(join(configDir, 'cocodeai'), { recursive: true })
writeFileSync(join(configDir, 'cocodeai', 'providers.json'), JSON.stringify({
  schemaVersion: 1,
...
setup_browser() {
  ...
  "${ab[@]}" eval "
    localStorage.setItem('cocodeai-open-tabs', JSON.stringify({
      openTabs: [{ sessionId: '${session_id}', title: '${title}', type: 'session' }],
      activeTabId: '${session_id}',
    }));
    localStorage.setItem('cocodeai-session-runtime', JSON.stringify({
