...
APP_BUNDLE_NAME="CocodeAI.app"
APP_BUNDLE_ID="com.cocodeai.desktop"

usage() {
  cat <<'EOF'
Build CocodeAI desktop for macOS Apple Silicon and output a DMG.
...
#   2) Tauri/Rust target 缓存复用旧 cocodeai-desktop,导致新 dist 没被嵌进去
...
if [[ "${PRESERVE_TAURI_TARGET:-0}" == "1" ]]; then
  ...
  rm -f "${DESKTOP_DIR}/src-tauri/target/${TARGET_TRIPLE}/release/cocodeai-desktop"
  rm -f "${DESKTOP_DIR}/src-tauri/target/release/cocodeai-desktop"
  find "${DESKTOP_DIR}/src-tauri/target/${TARGET_TRIPLE}/release/build" \
    -maxdepth 1 -name 'cocodeai-desktop-*' -exec rm -rf {} + 2>/dev/null || true
...
build_canonical_dmg() {
  ...
  staging_dir="$(mktemp -d "${TMPDIR:-/tmp}/cocodeai-dmg.XXXXXX")"
  rw_dmg="$(mktemp "${TMPDIR:-/tmp}/cocodeai-rw.XXXXXX").dmg"
...
if [[ -n "${LATEST_APP}" ]]; then
  ...
  # Without this, opening target/.../bundle/macos/CocodeAI.app directly
  ...
  CANONICAL_DMG="${CANONICAL_OUTPUT_DIR}/$(basename "${LATEST_DMG:-CocodeAI_0.1.0_aarch64.dmg}")"
  build_canonical_dmg \
