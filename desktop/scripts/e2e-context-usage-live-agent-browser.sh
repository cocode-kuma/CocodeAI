...
SESSION_NAME="cocodeai-context-live-${RUN_ID}"
ARTIFACT_DIR="${ARTIFACT_DIR:-$(mktemp -d "/tmp/cocodeai-context-live-${RUN_ID}-XXXX")}"
...
"${AB[@]}" eval "localStorage.setItem('cocodeai-locale', 'en'); localStorage.setItem('cocodeai-open-tabs', '${TAB_STATE}'); location.reload();" >>"${BROWSER_LOG}" 2>&1
