...
function Get-StagedArtifactName {
  ...
  switch -Regex ($ArtifactName) {
    ...
    '\.msi\.zip\.sig$' { return "CocodeAI_${appVersion}_windows_x64_msi.msi.zip.sig" }
    '\.msi\.zip$' { return "CocodeAI_${appVersion}_windows_x64_msi.msi.zip" }
    '\.msi\.sig$' { return "CocodeAI_${appVersion}_windows_x64_msi.msi.sig" }
    '\.msi$' { return "CocodeAI_${appVersion}_windows_x64_msi.msi" }
...
if (-not $env:TAURI_SIGNING_PRIVATE_KEY) {
  $tempConfigPath = Join-Path ([System.IO.Path]::GetTempPath()) 'cocodeai.tauri.local.windows.json'
  $tempConfig = @{
    bundle = @{
