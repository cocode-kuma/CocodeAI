...
```bash
./bin/cocodeai                          # Interactive TUI mode
./bin/cocodeai -p "your prompt here"    # Headless mode
./bin/cocodeai --help                   # Show all options
```
...
```bash
CLAUDE_CODE_FORCE_RECOVERY_CLI=1 ./bin/cocodeai
```
